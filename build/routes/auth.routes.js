...
const {
  ...
  isAdmin,
  isAutorized
} = require("../middlewares/auth.Jwt.js"); ///Route to create new user///


router.post("/Signup/:CompanyId", [checkDuplicateUsernameorEmail], //for the first user disble (verifyToken, isAutorized and isAdmin)
verifyToken, isAutorized, isAdmin, signUp); ///Route to create a new Role///

router.post("/NewRole/:CompanyId", verifyToken, isAdmin, checkDuplicateRole, newRole); ///Route to update user///
...
router.get("/Roles", verifyToken, getRoles); ///Route to get companies///

router.get("/Companies", verifyToken, getCompany); ///Route to login///

router.post("/Signin/:CompanyId", signIn); ///Route to have directory access///
