...
const path = require('path'); // --- MODELOS DE BASE DE DATOS ---
...
const {
  uploadToS3FromUrl
} = require('./uploadDiscord'); // --- VARIABLES DE ENTORNO Y CONSTANTES ---
...
const edicionSesion = new Map(); // tomaQuizSesion: Controla el estado de un alumno tomando un examen

const tomaQuizSesion = new Map(); // userState: Controla estados generales de navegación
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'setup-bienvenida') {
    ...
    const canal = interaction.channel; // Variables maestras para enviar al final

    let payloadEmbeds = [];
    let payloadComponents = []; // CASO 1: CANAL VINCULAR

    if (canal.name.includes('vincular')) {
      const embed = new EmbedBuilder().setTitle("👋 Bienvenido a Axiom Academy").setDescription("**Identificación Requerida**\n\n" + "Para acceder a los cursos, necesitamos validar tu número de empleado.\n" + "Haz clic en el botón de abajo para comenzar.").setColor('Green') // Asegúrate de poner aquí un link de imagen válido si quieres banner
      ...
      const embed = new EmbedBuilder().setTitle("📘 Catálogo de Cursos").setDescription("Aquí encontrarás la oferta académica disponible para tu perfil.").addFields({
        ...
        name: 'Instrucción',
        value: "Ejecuta el comando para desplegar el menú interactivo."
      }).setColor('Blue');
      payloadEmbeds = [embed];
    } // CASO 3: AULA
    else if (canal.name.includes('aula') || canal.name.includes('clase') || canal.name.includes('mi-aula')) {
      const embed = new EmbedBuilder().setTitle("🏫 Tu Aula Virtual").setDescription("Espacio dedicado a tu aprendizaje.").addFields({
        name: '▶️ Retomar / Ver Estado',
        value: "Usa `/mis-cursos`"
      }, {
        name: '💡 Tips',
        value: "Haz clic en el video para ampliar. Usa audífonos."
      }).setColor('Green');
      payloadEmbeds = [embed];
    } // CASO 4: CREADOR
    else if (canal.name.includes('creador')) {
      const embed = new EmbedBuilder().setTitle("🛠️ Estudio de Creación").setDescription("Espacio para Instructores.").addFields({
        name: '✨ Crear',
        value: "`/crear-curso`"
      }, {
        name: '🔄 Actualizar',
      ...
      const embed = new EmbedBuilder().setTitle("🚨 Centro de Control").setDescription("Panel administrativo.").addFields({
        name: '👥 Gestión',
        value: "`/asignar-curso`\n`/link-acceso`"
      }, {
        name: '🎓 Certificados',
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'modal_vincular_cuenta') {
    ...
    try {
      ...
      await employee.save(); // Asignar Rol
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'ver-cursos') {
    try {
      ...
      if (!categorias.length) return interaction.reply({
        content: "📂 No hay cursos.",
        ephemeral: true
      });
      const menu = new StringSelectMenuBuilder().setCustomId('menu_seleccionar_categoria').setPlaceholder('📂 Selecciona Área').addOptions(categorias.map(c => new StringSelectMenuOptionBuilder().setLabel(c).setValue(c).setEmoji('📁')));
      await interaction.reply({
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === 'menu_seleccionar_categoria') {
    ...
    const menu = new StringSelectMenuBuilder().setCustomId('menu_inscribir_curso').setPlaceholder(`📘 Cursos de ${cat}`).addOptions(cursos.map(c => new StringSelectMenuOptionBuilder().setLabel(c.title).setValue(c._id.toString()).setEmoji('🎓')));
    await interaction.update({
      components: [new ActionRowBuilder().addComponents(menu)]
    });
  } // INSCRIBIRSE
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === 'menu_inscribir_curso') {
    ...
    if (emp.enrolledCourses.find(c => c.status === 'In Progress')) {
      return interaction.reply({
        content: "⚠️ Termina tu curso actual primero.",
        ephemeral: true
      });
    }
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'asignar-curso') {
    ...
    if (!emp) return interaction.reply({
      content: "❌ Empleado no encontrado.",
      ephemeral: true
    });
    const cats = await Course.distinct('category', {
      isActive: true
    });
    const menu = new StringSelectMenuBuilder().setCustomId(`admin_assign_cat_${emp._id}`).setPlaceholder('Selecciona Área').addOptions(cats.map(c => new StringSelectMenuOptionBuilder().setLabel(c).setValue(c)));
    await interaction.reply({
      content: `Asignando a: **${emp.name}**`,
      components: [new ActionRowBuilder().addComponents(menu)],
      ephemeral: true
    });
  }
  ...
  if (interaction.isStringSelectMenu() && interaction.customId.startsWith('admin_assign_cat_')) {
    ...
    const menu = new StringSelectMenuBuilder().setCustomId(`admin_assign_final_${empId}`).setPlaceholder('Selecciona Curso').addOptions(cursos.map(c => new StringSelectMenuOptionBuilder().setLabel(c.title).setValue(c._id.toString())));
    await interaction.update({
      content: "Selecciona el curso:",
      components: [new ActionRowBuilder().addComponents(menu)]
    });
  }
  ...
  if (interaction.isStringSelectMenu() && interaction.customId.startsWith('admin_assign_final_')) {
    ...
    await emp.save(); // Notificar DM

    if (emp.discordId) {
      try {
        const user = await client.users.fetch(emp.discordId);
        const embed = new EmbedBuilder().setTitle("🚨 Nueva Asignación").setDescription(`Te han asignado: **${curso.title}**`).setColor('Red');
        await user.send({
          embeds: [embed]
        });
      } catch (e) {}
    ...
    await interaction.update({
      content: `✅ Asignado: **${curso.title}** a ${emp.name}.`,
      components: []
    });
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'ayuda') {
    ...
    embed.addFields({
      name: 'Estudiantes',
      value: "`/ver-cursos`, `/mis-cursos`"
    });
    if (esStaff) embed.addFields({
      name: 'Staff',
      value: "`/crear-curso`, `/asignar-curso`, `/setup-bienvenida`, `/admin-reporte`"
    });
    await interaction.reply({
      embeds: [embed],
      ephemeral: true
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'admin-reporte') {
    ...
    const menu = new StringSelectMenuBuilder().setCustomId('reporte_sel').setPlaceholder('Elige curso').addOptions(cursos.map(c => new StringSelectMenuOptionBuilder().setLabel(c.title).setValue(c._id.toString())));
    await interaction.reply({
      content: "Selecciona curso para reporte:",
      components: [new ActionRowBuilder().addComponents(menu)],
      ephemeral: true
    });
  }
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === 'reporte_sel') {
    ...
    const emps = await Employee.find({
      'enrolledCourses.courseId': cursoId
    });
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'test-certificado') {
    ...
    await interaction.reply({
      files: [new AttachmentBuilder(pdf, {
        name: 'test.pdf'
      })],
      ephemeral: true
    });
...
client.on('messageCreate', async message => {
  ...
  if (sesion.modo === 'ESPERANDO_MEDIA') {
    ...
    curso.phases.push({
      ...
      type: 'MEDIA',
      mediaUrl: s3.url,
      textContent: message.content || "Material de clase"
    });
    await curso.save();
    sesion.modo = 'MENU';
  ...
  if (sesion.modo === 'CREANDO_QUIZ') {
    ...
    if (txt === 'TERMINAR') {
      ...
      if (sesion.tempQuiz.esExamenFinal) c.finalExamId = q._id;else c.phases.push({
        order: c.phases.length + 1,
        title: "Quiz",
        type: 'QUIZ',
        quizId: q._id
      });
    ...
    if (sesion.tempQuiz.step === 0) {
      ...
      if (match) {
        ...
        sesion.tempQuiz.preguntas.push({
          text: sesion.tempQuiz.tempP,
          options: opts,
          correctIndex: parseInt(match[2]) - 1
        });
...
async function actualizarPanelFisico(interaction, cursoId) {
  ...
  const row1 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('btn_add_video').setLabel('Video').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('btn_add_img').setLabel('Imagen').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('btn_add_quiz').setLabel('Quiz').setStyle(ButtonStyle.Secondary));
  const row2 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('btn_add_final_exam').setLabel('Examen Final').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId('btn_publicar').setLabel('PUBLICAR').setStyle(ButtonStyle.Success));
  await interaction.editReply({
    embeds: [embed],
...
async function mostrarFaseReproductor(interaction, cursoId, index) {
  ...
  if (index >= curso.phases.length) {
    // Fin del curso
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`start_exam_${curso.finalExamId}`).setLabel('Examen Final').setStyle(ButtonStyle.Success));
    return interaction.message ? interaction.update({
  ...
  if (isVideo) {
    ...
    files = [new AttachmentBuilder(fase.mediaUrl, {
      name: 'video.mp4'
    })];
  } else {
  ...
  const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`player_prev_${cursoId}_${index}`).setLabel('⬅️').setStyle(ButtonStyle.Secondary).setDisabled(index === 0));
...
async function mostrarPreguntaExamen(interaction) {
  ...
  const embed = new EmbedBuilder().setTitle(`🧠 Pregunta ${sesion.indicePregunta + 1}`).setDescription(`\`\`\`fix\n${p.text}\n\`\`\``).setColor('Orange');
...
async function finalizarExamen(interaction, sesion) {
  ...
  const passed = score >= NOTA_MINIMA_APROBATORIA; // Guardar intento
  ...
  if (passed && sesion.esExamenFinal) {
    ...
    try {
      ...
      const att = new AttachmentBuilder(pdf, {
        name: 'Diploma.pdf'
      });
      await interaction.user.send({
        content: "🎓 ¡Felicidades! Aquí tienes tu certificado:",
        files: [att]
      });
    } catch (e) {
    ...
    await interaction.update({
      content: `🏆 **¡APROBADO!** Nota: ${score}%`,
      embeds: [],
      components: []
    });
  } else {
    await interaction.update({
      content: `❌ **Reprobado**. Nota: ${score}%. Inténtalo de nuevo.`,
      embeds: [],
      components: []
    });
  }
...
async function crearPDFCertificado(nombre, curso, fecha) {
  ...
  const rutaFuente = path.join(__dirname, '../assets/fuente_nombre.ttf'); // Si no tienes assets, crea un PDF blanco básico para que no crashee

  if (!fs.existsSync(rutaTemplate) || !fs.existsSync(rutaFuente)) {
    const doc = await PDFDocument.create();
    const page = doc.addPage();
    page.drawText(`${nombre} - ${curso}`, {
      x: 50,
      y: 400
    });
