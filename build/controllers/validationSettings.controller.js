const ValidationSettings = require("../models/ValidationSettings.js");
...
const AssemblyStartTechnical = require("../models/AssemblyStartTechnical.js");

const AssemblyStartProduction = require("../models/AssemblyStartProduction.js");

const AssemblyStartQuality = require("../models/AssemblyStartQuality.js");

const TemporalStop = require("../models/TemporalStop.js");

const EndRun = require("../models/EndRun.js"); //method to create new validation settings////////////////////////////////////////////////////////////////////////
...
const createValidationSettings = async (req, res) => {
  ...
  res.json({
    status: "200",
    message: "validation settings created",
    savedValidationSettings
  });
}; //method to update validation settings///////////////////////////////////////////////////////////////////////////////////////////////
...
const updateValidationSetting = async (req, res) => {
  ...
  const foundAssemblyStartTechnical = await AssemblyStartTechnical.find({
    noCheckValidation: {
      $in: req.body.assemblyStartTechnical
    }
  });
  ...
  const foundAssemblyStartQuality = await AssemblyStartQuality.find({
    noCheckValidation: {
      $in: req.body.assemblyStartQuality
    }
  });
  ...
  const foundAssemblyStartProduction = await AssemblyStartProduction.find({
    noCheckValidation: {
      $in: req.body.assmeblyStartProduction
    }
  });
  ...
  if (!updatedValidationSetting) {
    res.status(403).json({
      status: "403",
      message: "validqation settings not updated",
      body: ""
    });
  }
...
const getValidationSetting = async (req, res) => {
  ...
  res.json({
    status: "200",
    message: "validationSetting loaded",
    body: validationSetting
  });
};
