...
const getTrialEvaluations = async (req, res) => {
  ...
  res.json({
    status: "200",
    message: "Trial Evaluations Loaded",
    body: trialEvaluations
  });
};
...
const createNewTrialEvaluation = async (req, res) => {
  try {
    ...
    if (count > 1) {
      const trialEvaluations = await TrialEvaluations.find().sort({
        consecutive: -1
      }).limit(1);
    ...
    res.status(500).json({
      message: "Ocurrió un error interno al guardar la evaluación."
    }); // Para el cliente
  }
};
...
const updateTrialEvaluation = async (req, res) => {
  ...
  try {
    ...
    if (error.message === "User not found") {
      return res.status(404).json({
        status: "error",
        message: "Uno o más usuarios no fueron encontrados."
      });
    }
...
const updateEmployeeSignature = async (req, res) => {
  ...
  const foundPrevEvaluation = await TrialEvaluations.findById(TrialEvaluationID); // Deleting Images from Folder
