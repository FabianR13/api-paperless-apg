...
const s3 = new AWS.S3(); //create deviation request//////////////////////////////////////////////////////////////////////////////////////
...
const createDeviation = async (req, res, next) => {
  ...
  try {
    ...
    if (productResponsible && productResponsible.length > 0) {
      ...
      const countFound = await Employees.countDocuments({
        _id: {
          $in: uniqueIds
        }
      });
      ...
      if (countFound !== uniqueIds.length) {
        return res.status(404).json({
          status: "error",
          message: "Uno o más IDs de empleado no fueron encontrados."
        });
      }

      const responsablesParaGuardar = productResponsible.map(id => ({
        employee: id
      }));
    ...
    if (processResponsible && processResponsible.length > 0) {
      ...
      const responsablesParaGuardar = processResponsible.map(id => ({
        employee: id
      }));
    ...
    if (correctiveResponsible && correctiveResponsible.length > 0) {
      ...
      const responsablesParaGuardar = correctiveResponsible.map(id => ({
        employee: id
      }));
    ...
    if (preventiveResponsible && preventiveResponsible.length > 0) {
      ...
      const responsablesParaGuardar = preventiveResponsible.map(id => ({
        employee: id
      }));
    ...
    if (followCorrectiveResponsible && followCorrectiveResponsible.length > 0) {
      ...
      const responsablesParaGuardar = followCorrectiveResponsible.map(id => ({
        employee: id
      }));
    ...
    if (followPreventiveResponsible && followPreventiveResponsible.length > 0) {
      ...
      const responsablesParaGuardar = followPreventiveResponsible.map(id => ({
        employee: id
      }));
    ...
    next(); //res.json({ status: "200", message: "Deviation request created", savedDeviationRequest });
  } catch (error) {
    console.error(error);
    res.status(500).json({
      status: "error",
...
const updateDeviation = async (req, res) => {
  ...
  let newFollowPreventiveResponsible = ""; // Buscar customer en la base de datos
...
const validateDeviation = async (req, res) => {
  ...
  try {
    ...
    if (Object.keys(fieldsToUpdate).length === 0) {
      return res.status(400).json({
        message: "El usuario no tiene los roles necesarios para firmar en este paso."
      });
    }
    ...
    if (updateResult.matchedCount === 0) {
      return res.status(404).json({
        message: "La desviación a actualizar no fue encontrada."
      });
    }

    res.status(200).json({
      status: "200",
      message: "Desviación actualizada correctamente."
    });
  } catch (error) {
