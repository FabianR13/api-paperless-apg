const RewardsKaizen = require("../models/RewardsKaizen"); // Función auxiliar para obtener el siguiente consecutivo


const getNextConsecutive = async category => {
  const lastReward = await RewardsKaizen.findOne({
    category: category
  }).sort({
    consecutive: -1
  }).limit(1);
...
const createReward = async (req, res) => {
  try {
    ...
    const {
      num,
      full
    } = await getNextConsecutive(category);
...
const updateReward = async (req, res) => {
  try {
    ...
    if (!rewardToUpdate) return res.status(404).json({
