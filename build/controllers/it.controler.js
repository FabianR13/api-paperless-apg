...
const uploadLaptopLetter = async (req, res) => {
  const {
    laptopId
  } = req.params; //Getting Previous document
  ...
  let modified = req.body.modified; // Updating the new Img Names in the fields from the DB
...
const createNewGenericAccount = async (req, res) => {
  ...
  if (!saveGenericAccount) {
    res.status(403).json({
      status: "403",
      message: "Generic Account not Saved",
      body: ""
    });
  }
...
const uploadCellphoneLetter = async (req, res) => {
  ...
  const foundPrevCellphone = await Cellphones.findById(cellphoneId); // Deleting Images from Folder

  const prevCellphoneLetter = foundPrevCellphone.responsibeLetter; // Validating if there are Images in the Field
...
const uploadAccountsLetter = async (req, res) => {
  ...
  const foundPrevAccounts = await Accounts.findById(accountsId); // Deleting Images from Folder

  const prevAccountsLetter = foundPrevAccounts.responsibeLetter; // Validating if there are Images in the Field
