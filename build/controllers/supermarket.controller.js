...
const Devolucion = require("../models/Devoluciones.js");

const User = require("../models/User.js");

const RegistroMovimientos = require("../models/RegistroMovimiento.js"); //Crear nuevos Items//////////////////////////////////////////////////////////////////////////////////////////////////// 
...
const createItems = async (req, res) => {
  const {
    materials,
    user
  } = req.body; // 'materials' es el array que proporcionaste
    ...
    try {
      ...
      if (foundCompany) {
        ...
        return res.status(404).json({
          message: "Compañía no encontrada con el ID proporcionado"
        });
      }
    } catch (error) {
      console.error("Error al buscar compañía:", error);
      return res.status(500).json({
        message: "Error al buscar la compañía"
      });
    }
  ...
  if (!Array.isArray(materials)) {
    return res.status(400).json({
      message: "Datos inválidos, se esperaba un array de materiales"
    });
  ...
  if (user) {
    // 'user' probablemente sea el username
    try {
      const foundUser = await User.findOne({
        username: user
      }); // findOne si esperas un solo usuario

      if (foundUser) {
        nuevoMovimiento.usuario = foundUser._id;
      } else {
        console.warn(`Usuario "${user}" no encontrado para el registro de movimiento.`); // Decide si esto es un error o si el movimiento puede guardarse sin usuario.
      }
    } catch (error) {
      console.error("Error al buscar usuario:", error); // Considera cómo manejar este error.
    }
  }
  ...
  try {
    ...
    for (const material of materials) {
      ...
      const existingMaterial = await Items.findOne({
        name: material.name,
        company: companyObjectId
      }); // Opcional: buscar también por compañía si es relevante
      ...
      if (existingMaterial) {
        ...
        if (material.itemGroup) {
          ...
          existingMaterial.itemGroup.valor = material.itemGroup; // existingMaterial.itemGroup.descripcion = "Actualizado"; // O alguna lógica para la descripción
        ...
        const newMaterialData = { ...material,
          // Copia todas las propiedades del material de entrada
          qty: material.qty || 0,
          // Asegurar que qty tenga un valor numérico
          itemGroup: {
            valor: material.itemGroup || "Desconocido",
            // Valor predeterminado si no viene
    ...
    res.status(200).json({
      status: "200",
      message: 'Materiales procesados correctamente'
    });
  } catch (error) {
    console.error('Error al procesar materiales:', error);
    res.status(500).json({
      message: 'Error interno del servidor al procesar materiales',
...
const createPedido = async (req, res) => {
  ...
  const dateMX = new Date(new Date().toLocaleString("en-US", {
    timeZone: "America/Mexico_City"
  }));
  ...
  const idPedidoPrefix = `PED-${newDate}-${turno}`; // 2. Buscar el último pedido creado

  const lastPedido = await Pedido.findOne().sort({
    createdAt: -1
  });
  let consecutivo = 1;

  if (lastPedido && lastPedido.idPedido) {
    const lastId = lastPedido.idPedido; // --- EXPRESIÓN REGULAR MEJORADA ---
  ...
  try {
    const {
      ...
      maquina,
      materiales
    } = req.body.pedidos; // Validaciones básicas
    ...
    if (materiales.length > 0) {
      const names = materiales.map(mat => mat.name);
      const foundMaterials = await Items.find({
        name: {
          $in: names
        }
      });
      ...
      if (!foundMaterials || foundMaterials.length === 0) {
        return res.status(404).json({
          status: "error",
          message: "No se encontraron materiales en la base de datos"
        });
...
const updatePedido = async (req, res) => {
  ...
  try {
    ...
    pedidoActual.items = items.map(material => ({
      ...
      // Actualizar los seriales
      quantityReq: material.quantityReq,
      // Mantener el quantityReq original si es necesario
      quantitySur: material.quantitySur,
    ...
    pedidoActual.pStatus = pStatus; // Guardar cambios en el pedido

    await pedidoActual.save(); // Guardar el movimiento registrado

    await nuevoMovimiento.save(); // Enviar respuesta de éxito
...
const getRecentPedidos = async (req, res) => {
  const {
    CompanyId
  } = req.params; // Valida la longitud del CompanyId (asumiendo que es un ObjectId de MongoDB)

  if (CompanyId.length !== 24) {
    // Es mejor enviar una respuesta en lugar de solo retornar
    return res.status(400).json({
      status: "400",
      message: "Formato de CompanyId inválido."
    });
  }
  ...
  try {
    ...
    const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1000); // Busca los pedidos

    const pedidos = await Pedido.find({
      company: CompanyId,
      // Asumo que 'company' en Pedido es el ObjectId de la compañía.
      // Si CompanyId pudiera ser un array, { $in: CompanyId } sería correcto.
      createdAt: {
        $gte: twentyFourHoursAgo
      } // Filtra los pedidos creados desde hace 24 horas hasta ahora.
...
const cancelPedido = async (req, res) => {
  ...
  try {
    ...
    const {
      ...
      pStatus,
      cancelReason
    } = req.body.pedido; // Buscar el pedido actual
    ...
    pedidoActual.surTime = time;
    pedidoActual.pStatus = pStatus;
    pedidoActual.cancelReason = cancelReason; // Guardar cambios en el pedido
...
const createDevolucion = async (req, res) => {
  ...
  const hourMX = dateMX.getHours();
  const minutesMX = dateMX.getMinutes();
  const time = `${String(hourMX).padStart(2, '0')}:${String(minutesMX).padStart(2, '0')}`; // 2. Determinar Turno

  let turno;
  if (hourMX >= 7 && hourMX < 15) turno = 'D';else if (hourMX >= 15 && hourMX < 23) turno = 'A';else turno = 'N'; // 3. Generar ID Único para Devolución

  const idPrefix = `DEV-${newDate}-${turno}`; // --- CAMBIO: Buscamos en el modelo Devolucion, no en Pedido ---

  const lastDevolucion = await Devolucion.findOne({
    idDevolucion: {
      $regex: new RegExp(`^${idPrefix}`)
    }
  }).sort({
    createdAt: -1
  });
  ...
  const paddedConsecutivo = String(consecutivo).padStart(3, '0');
  let idDevolucion = `${idPrefix}${paddedConsecutivo}`; // Validación duplicados turno N (lógica de seguridad)
  ...
  try {
    ...
    const {
      CompanyId
    } = req.params; // Necesitamos el ID de la empresa
    ...
    }).filter(Boolean); // --- CAMBIO: Instanciar nuevo modelo Devolucion ---
...
const getDevoluciones = async (req, res) => {
  ...
  try {
    ...
    .limit(100).populate({
      ...
      path: "items.id",
      // Nota: en el esquema items -> id es la ref
      select: "name description vendorItemNo"
...
const updateDevolucionStatus = async (req, res) => {
  ...
  const {
    ...
    usuario,
    reason
  } = req.body; // status: 'confirmed', 'validated', 'rejected'
  ...
  try {
    ...
    if (!devolucion) return res.status(404).json({
      status: "error",
    ...
    if (!actionUser) return res.status(404).json({
      status: "error",
      message: "Usuario no encontrado"
    ...
    switch (status) {
      case 'confirmed':
        ...
        if (devolucion.dStatus !== 'started' && devolucion.dStatus !== 'rejected') {
          return res.status(400).json({
            status: "error",
            message: "No se puede confirmar en el estado actual."
          });
        }

        devolucion.dStatus = 'confirmed';
        devolucion.confirmedBy = actionUser._id; // Si venía de rechazo, limpiamos el motivo anterior para indicar que se corrigió

        if (devolucion.dStatus === 'rejected') {// Opcional: Podrías mantener el historial, pero aquí limpiamos para "nueva confirmación"
      ...
      case 'validated':
        if (devolucion.dStatus !== 'confirmed') {
          return res.status(400).json({
            status: "error",
            message: "Solo se pueden validar devoluciones confirmadas."
          });
        }
      ...
      case 'rejected':
        if (devolucion.dStatus !== 'confirmed') {
          return res.status(400).json({
            status: "error",
            message: "Solo se pueden rechazar devoluciones confirmadas."
          });
        }
...
const updateDevolucionContent = async (req, res) => {
  const {
    idDevolucion
  } = req.params; // Recibimos: usuario (quien edita) y items (la nueva lista corregida)
  ...
  try {
    ...
    if (devolucion.dStatus !== 'rejected' && devolucion.dStatus !== 'started') {
      return res.status(400).json({
        status: "error",
        message: "No se puede editar una devolución en este estado."
      });
    ...
    }).filter(Boolean); // Actualizar datos

    devolucion.items = mappedItems; // Opcional: Si quieres que al editar se pase automáticamente a 'started' o se quede en 'rejected'
