    ...
    for (const group of materials) {
      for (const material of group.materials) {
        ...
        if (existingMaterial) {
          ...
          const newMaterial = new Items({ ...material,
            itemGroup: {
              valor: material.itemGroup,
              // Asignamos el valor
...
const createPedido = async (req, res) => {
  try {
    ...
    const nuevoMovimiento = new RegistroMovimientos({
      ...
      detalles: `Creo el pedido con id:  ${idPedido}}.`
    }); // Buscar ID del usuario en la base de datos
    ...
    if (usuario) {
      ...
      nuevoPedido.usuario = foundUsers.map(user => user._id);
      nuevoMovimiento.usuario = foundUsers.map(user => user._id);
    } // Buscar IDs de los materiales en la base de datos


    if (materiales.length > 0) {
      // Buscar los materiales en la base de datos por nombre
      const foundMaterials = await Items.find({
        name: {
          $in: materiales.map(mat => mat)
        }
      }); // Validar que se encontraron materiales

      if (!foundMaterials || foundMaterials.length === 0) {
        return res.status(400).json({
          status: "error",
          message: "No se encontraron los materiales en la base de datos."
        });
      } // Asignar los materiales encontrados con sus IDs


      nuevoPedido.items = foundMaterials.map(material => ({
        id: material._id,
        // Guardar el ID del material
        serial: [],
        // Array vacío
        quantityReq: 1,
        quantitySur: [] // Array vacío
  ...
  try {
    ...
    for (const item of items) {
      const {
        id,
        serial
      } = item; // serial es un array de seriales

      console.log(`Verificando ID: ${id}, Seriales: ${serial}`); // Buscar si alguno de los seriales ya está registrado en otro pedido
      ...
      const pedidoExistente = await Pedido.findOne({
        ...
        // Busca si algún serial ya existe en otro pedido
        _id: {
          $ne: pedidoActual._id
        } // Excluir el pedido actual
      ...
      if (pedidoExistente) {
        ...
        nuevoMovimiento.tipoAccion = "Ingresar duplicado", nuevoMovimiento.detalles = `Alguno de estos seriales "${serialDuplicado || serial}" ya está registrado en el pedido con ID "${pedidoExistente.idPedido}".`;
        await nuevoMovimiento.save();
        return res.status(400).json({
