    ...
    try {
        ...
        if (materiales.length > 0) {
            ...
            const foundMaterials = await Items.find({
                name: { $in: materiales.map(mat => mat) }
            });

            // Validar que se encontraron materiales
            if (!foundMaterials || foundMaterials.length === 0) {
                return res.status(400).json({ status: "error", message: "No se encontraron los materiales en la base de datos." });
            }

            // Asignar los materiales encontrados con sus IDs
            nuevoPedido.items = foundMaterials.map(material => ({
                id: material._id,  // Guardar el ID del material
                serial: [],         // Array vacío
                quantityReq: 1,
