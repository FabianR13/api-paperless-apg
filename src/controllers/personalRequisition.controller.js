const PersonalRequisition = require("../models/PersonalRequisition.js");
...
const createPersonalRequisition = async (req, res, next) => {
    ...
    if (autorizedByFinance) {
        const foundUsers = await User.find({
            username: { $in: autorizedByFinance },
        });
...
const getAllPersonalRequisitions = async (req, res) => {
    ...
    const personalRequisitions = await PersonalRequisition.find({
        ...
        .populate({ path: 'requestBy', select: "username", populate: [{ path: "signature", select: ("signature") }, { path: "employee", select: ("name lastName numberEmployee"), populate: { path: "department position", select: "name" } }] })
        .populate({ path: 'recruiter', select: "username signature email", populate: { path: "employee", select: ("name lastName numberEmployee"), populate: { path: "department position", select: "name" } } })
        ...
        .populate({ path: 'autorizedByFinance', select: "username", populate: [{ path: "signature", select: ("signature") }, { path: "employee", select: ("name lastName numberEmployee"), populate: { path: "department", select: "name" } }] })
        .populate({ path: 'autorizedByGeneral', select: "username", populate: [{ path: "signature", select: ("signature") }, { path: "employee", select: ("name lastName numberEmployee"), populate: { path: "department", select: "name" } }] })
        .populate({ path: 'hrSignature', select: "username", populate: [{ path: "signature", select: ("signature") }, { path: "employee", select: ("name lastName numberEmployee"), populate: { path: "department", select: "name" } }] });
...
const getRequisitionById = async (req, res) => {
    const foundRequisition = await PersonalRequisition.findById(req.params.requisitionId)
        .populate({ path: 'requestBy', select: "username email", populate: [{ path: "signature", select: ("signature") }, { path: "employee", select: ("name lastName numberEmployee"), populate: { path: "department position", select: "name" } }] })
        .populate({ path: 'recruiter', select: "username email", populate: [{ path: "signature", select: ("signature") }, { path: "employee", select: ("name lastName numberEmployee"), populate: { path: "department position", select: "name" } }] })
        .populate({ path: 'internalPromotion', populate: { path: "department position", select: "name" } })
        .populate({ path: 'employeeCopy', populate: { path: "department position", select: "name" } })
        .populate({ path: 'peopleInvolved', populate: { path: "department position", select: "name" } })
        .populate({ path: 'autorizedByFinance', select: "username", populate: [{ path: "signature", select: ("signature") }, { path: "employee", select: ("name lastName numberEmployee"), populate: { path: "department", select: "name" } }] })
...
const UpdateRequisitionSignature = async (req, res, next) => {
    ...
    if (newAutorizedByFinance) {
        const foundUser = await User.find({
            username: { $in: newAutorizedByFinance },
        });
